...
var buildFabric = function (ticketId) {
        ...
        Fabric_Client.newDefaultKeyValueStore({
            ...
            if (query_responses && query_responses.length == 1) {
                if (query_responses[0] instanceof Error) {
                    ...
                    var QRcodeContent = ticketId + "$" + message + "$" + json.SchSign + "$";
                    console.log("QR code " + QRcodeContent)

                    var ticketArr = [message, QRcodeContent, json.Owner,
                        json.IssuedDate, json.ExpDate, json.SchSign]
...
router.post('/', function (req, res, next) {
    ...
    buildFabric(ticketId).then((result) => {
        if (req.headers['mobile']) {
            return res.status(200).json({ status: "ok", message: result });
        }
        else {
            // res.render('student/createQrcode', {
            //     ticketMessage: result[0],
            //     ticketId: result[1],
            //     owner: result[2],
            //     issuedDate: result[3],
            //     expDate: result[4],
